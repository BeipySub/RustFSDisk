...
test("accepts scan websocket events and updates scan summary without fake disks", () => {
  const summary = normalizeEdgeDashboardSummary({
    source: "edge",
    edge_code: "edge-demo",
    edge_name: "edge-demo",
  ...
  const event = parseCopyProgressEvent(
    JSON.stringify({
      event_type: "SCAN_PROGRESS",
      event_time: "2026-08-11T08:28:00Z",
      source: "edge",
      edge_code: "edge-demo",
      export_job_id: "",
      ...
      disks: [],
      message: "scanning rustfs",
    }),
  );

  assert.notEqual(event, null);
  const merged = applyCopyProgressEvent(summary, event);
  assert.equal(merged.scan.scan_event_type, "SCAN_PROGRESS");
  assert.equal(merged.scan.scanned_object_count, 12);
  assert.equal(merged.scan.stable_object_count, 9);
  assert.equal(merged.scan.scanned_bytes, 2048);
  assert.equal(merged.disks.length, 0);
});

test("uses physical disk fields as UI identity for detected disks without disk_id", () => {
  const summary = normalizeEdgeDashboardSummary({
    source: "edge",
    edge_code: "edge-demo",
    edge_name: "edge-demo",
      ...
      disks: [
        {
          ...
          object_remaining: 0,
          current_object: null,
          message: "disk runtime_status=DETECTED",
        },
      ],
      message: "detected",
    }),
  ...
  assert.equal(merged.disks[0]?.disk_id, "/media/edge/new");
  assert.equal(merged.disks[0]?.last_event_type, "DISK_DETECTED");
});
